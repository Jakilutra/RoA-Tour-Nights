...
=IFS(
        ...
        SUM(OFFSET($A3,2,0):OFFSET($A$1936,-2,0)) <> ((MAX(OFFSET($A3,2,0):OFFSET($A$1936,-2,0))^2+MAX(OFFSET($A3,2,0):OFFSET($A$1936,-2,0)))/2), "A Tour Night # is incorrect.",
        ...
        (COUNTBLANK(OFFSET($A$3,2,0):OFFSET($O$1936,-2,0))+COUNTBLANK(OFFSET($R$3,2,0):OFFSET($R$1936,-2,0))-166+COUNTBLANK(OFFSET($U$3,2,0):OFFSET($U$1936,-2,0)))>0, "Warning: Missing cell content detected.",
        IFERROR(VALUE(REGEXEXTRACT('Host Standings'!E5,"\d+"))<>(COUNTA(OFFSET($A$3,2,0):OFFSET(A$1936,-2,0))+COUNTBLANK(OFFSET($A$3,2,0):OFFSET($A$1936,-2,0))),TRUE)+IFERROR(REGEXEXTRACT('Host Standings'!C5,"\d+")<>REGEXEXTRACT(F4,"\d+"),TRUE)+IFERROR(REGEXEXTRACT('Host Standings'!G5,"\d+")<>REGEXEXTRACT(I4,"\d+"),TRUE)+IF(""&COUNTA('Host Standings'!C6:C)<>IFERROR(REGEXEXTRACT('Host Standings'!C5,"\d+"),COUNTA('Host Standings'!C6:C)),1,0), "Warning: Host Standings disparity! Please attempt updating the standings.",
        ...
        ARRAYFORMULA(SUM(IFERROR(S5:S,1)))>0, "Warning: Probably a length disparity between Winners, Tiers, Sizes and Replay Links arrays.",
         IFERROR(VALUE(REGEXEXTRACT(T4,"\d+"))+COUNTBLANK(OFFSET($T$3,2,0):OFFSET($T$1936,-2,0)),(COUNTIF(OFFSET($D$3,2,0):OFFSET($D$1936,-2,0),CEILING(MAX(OFFSET($D$3,2,0):OFFSET($D$1936,-2,0))/3)*3)+COUNTIF(OFFSET($D$3,2,0):OFFSET($D$1936,-2,0),(CEILING(MAX(OFFSET($D$3,2,0):OFFSET($D$1936,-2,0))/3)*3)-1)+COUNTIF(OFFSET($D$3,2,0):OFFSET($D$1936,-2,0),(CEILING(MAX(OFFSET($D$3,2,0):OFFSET($D$1936,-2,0))/3)*3)-2)+((FLOOR((OFFSET($D$1936,-2,0)-1)/3)*3)*7+18))) < (COUNTIF(OFFSET($D$3,2,0):OFFSET($D$1936,-2,0),CEILING(MAX(OFFSET($D$3,2,0):OFFSET($D$1936,-2,0))/3)*3)+COUNTIF(OFFSET($D$3,2,0):OFFSET($D$1936,-2,0),(CEILING(MAX(OFFSET($D$3,2,0):OFFSET($D$1936,-2,0))/3)*3)-1)+COUNTIF(OFFSET($D$3,2,0):OFFSET($D$1936,-2,0),(CEILING(MAX(OFFSET($D$3,2,0):OFFSET($D$1936,-2,0))/3)*3)-2)+((FLOOR((OFFSET($D$1936,-2,0)-1)/3)*3)*7+18)), "Warning: A row entry appears to be missing!",
        COUNTBLANK(OFFSET($T$3,2,0):OFFSET($T$1936,-2,0))>0, "Warning: Missing post URL detected. Post required on the forums. See Step 22.",
        ""&COUNTA(OFFSET($D$3,2,0):OFFSET($D$1936,-2,0))<>IFERROR(REGEXEXTRACT(T4,"\d+"),COUNTA(OFFSET($D$3,2,0):OFFSET($D$1936,-2,0))), "Warning: Duplicate linked post",
